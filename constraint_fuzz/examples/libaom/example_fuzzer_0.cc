...
//<ID> 2
//<Prompt>
//["aom_codec_err_to_string","aom_uleb_decode","aom_img_metadata_free","aom_codec_enc_config_default","aom_codec_get_frame"]
/*<Combination>: [const char *aom_codec_err_to_string(aom_codec_err_t err),
    int aom_uleb_decode(const uint8_t * buffer, size_t available, uint64_t *
   value, size_t * length), void aom_img_metadata_free(aom_metadata_t *
   metadata), aom_codec_err_t aom_codec_enc_config_default(aom_codec_iface_t *
   iface, aom_codec_enc_cfg_t * cfg, unsigned int usage), aom_image_t
   *aom_codec_get_frame(aom_codec_ctx_t * ctx, aom_codec_iter_t * iter)
*/
//<score> 0.6, nr_unique_branch: 2, p_cov: 0.2
//<Quality>
//{"diversity":9,"density":9,"covered_api":["aom_codec_enc_config_default"],"uncovered_api":["aom_codec_err_to_string","aom_uleb_decode","aom_img_metadata_free","aom_codec_get_frame"],"unique_branches":{"aom_codec_enc_init_ver":[[68,12,68,40,0],[68,44,68,79,1]]},"library_calls":["aom_codec_av1_cx","aom_codec_enc_config_default","aom_codec_enc_init_ver","aom_img_alloc","aom_codec_encode","aom_codec_get_cx_data","aom_img_free","aom_codec_destroy"]}
/**/
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  // Initialize encoder
  aom_codec_err_t encoder_init_result =
      aom_codec_enc_init_ver(&encoder_ctx, encoder_iface, &encoder_cfg,
                             AOM_CODEC_USE_PSNR, AOM_ENCODER_ABI_VERSION);
  ...
  aom_image_t *frame = aom_img_alloc(NULL, AOM_IMG_FMT_I420, encoder_cfg.g_w,
                                     encoder_cfg.g_h, 32);
  if (!frame) {
  ...
  for (;;) {
    ...
    fwrite(encoded_data->data.frame.buf, 1, encoded_data->data.frame.sz,
           out_file);
  }
