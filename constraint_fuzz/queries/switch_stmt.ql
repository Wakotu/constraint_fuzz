...
select switchStmt.getLocation() as switch_stmt_location,
  ...
  stmt.getLocation() as case_stmt_location, getStmtType(stmt) as case_stmt_type,
  switchStmt.getEnclosingFunction().getName() as function_name,
  switchStmt.getFile().getAbsolutePath() as file_path
