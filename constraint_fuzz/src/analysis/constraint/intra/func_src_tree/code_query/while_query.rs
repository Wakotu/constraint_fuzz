...
pub type WhilePool = FileFuncTable<WhileSet>;
...
impl CodeQLRunner {
    pub fn get_while_pool(&self) -> Result<WhilePool> {
        ...
        let mut while_pool: WhilePool = FileFuncTable::new();
        for record in records.into_iter() {
            let while_set = while_pool.get_value_mut(&record.file_path, &record.func_name);
