...
use crate::analysis::constraint::inter::exec_tree::action::{
    get_prefix, ExecAction, FuncAction, FuncCallAction, JumpAction, LoopAction, SelectAction,
    ThreadAction,
};
...
impl<'a> FuncActIter<'a> {
    ...
    pub fn get_cur(&mut self) -> Option<&'a ExecAction> {
        while self.is_nostart_loopout() {
            self.idx += 1;
        }
        self.func_node.get_act_at(self.idx)
    }

    pub fn update(&mut self) {
        if self.idx < self.func_node.get_len() {
            self.idx += 1;
        }
    }
    ...
    pub fn next_res(&mut self) -> Result<&'a ExecAction> {
...
impl ExecThreadTree {
    ...
    fn parse_tid<P: AsRef<Path>>(fpath: P) -> Result<usize> {
        let fname = fpath
            ...
            .ok_or_else(|| eyre::eyre!("Failed to get filename from path: {:?}", fpath.as_ref()))?;
        let pos_op = fname.find('_');
        let tid = if let Some(pos) = pos_op {
            let tid_str = &fname[0..pos];
            tid_str.parse::<usize>()?
        } else {
            fname.parse::<usize>()?
        };
    ...
    //     line: &str,
    // ) -> std::result::Result<(UBVHit, JumpAction), GuardParseError> {
    ...
    //         // parse value hit
    //         let value_hit = UBVHit::from_str(value_hit_str)?;
    ...
    fn parse_actrec_impl(
            ...
            ActrecParseError::to_eyre(ThreadAction::parse_thread_act_rec(line))?
        {
            ...
            return Ok((Some(ExecAction::Thread(thread_act)), Some(thcp_entry)));
        }
            ...
            if let ExecAction::Func(func_act) = &act {
                match func_act {
                    FuncAction::Call(call_act) => {
                        self.cur_node_ptr = call_act.child_ptr.clone();
                        self.cur_depth += 1;
                    ...
                    FuncAction::Return { func_name } => {
                        ...
                        self.node_return(&func_name)?;
                    }
                }
            }

            if let Some(uw_act) = act.get_unwind_act() {
                self.node_return(&uw_act.func_name)?;
            }
    ...
    // pub fn from_guard_file_with_constraint<P: AsRef<Path>>(
    //     fs_path: P,
    //     cons: &Constraint,
    ...
    pub fn from_guard_file<P: AsRef<Path>>(
        ...
        let mut exec_tree: ExecThreadTree = ExecThreadTree::new(fs_path.as_ref())?;
...
pub struct ExecForest {
    main_tree: ExecThreadTree,
    sub_trees: Vec<ExecThreadTree>,
